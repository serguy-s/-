...
class ReceiptItem:
    ...
    def save(self):
        """Сохраняет позицию чека в базе данных."""
        conn = None
        try:
            conn = get_connection()
            cursor = conn.cursor()
            if self.item_id is None:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?)
                ''', (self.receipt_code, self.service_code, self.quantity, self.item_price))
                ...
                cursor.execute('''
                    UPDATE receipt_items SET 
                        ...
                        item_price = ?
                    WHERE item_id = ?
                ''', (self.receipt_code, self.service_code, self.quantity, self.item_price, self.item_id))
            conn.commit()
            ...
            if conn:
                conn.close()
    def delete(self):
        """Удаляет позицию чека из базы данных."""
        if not self.item_id:
            return
        conn = None
        try:
            conn = get_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM receipt_items WHERE item_id = ?", (self.item_id,))
            conn.commit()
        except Exception as e:
...
def get_receipt_items_by_receipt_code(receipt_code):
    """Возвращает позиции чека по коду чека."""
    conn = None
    try:
        conn = get_connection()
        cursor = conn.cursor()
...
def get_receipt_item_by_id(item_id):
    """Возвращает позицию чека по идентификатору позиции."""
    conn = None
    try:
        conn = get_connection()
        cursor = conn.cursor()
